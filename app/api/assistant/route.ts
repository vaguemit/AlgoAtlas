...
// Initialize the Groq client lazily to avoid build-time crash when env var is missing
...
export async function POST(req: Request) {
  try {
    ...
    - You are powered by the Groq API using ${preferredModel} (either llama-3.3-70b-versatile or llama-3.1-8b-instant model depending on user settings)
    ...
    - If asked about your parameters or specifications, state ONLY that you are running on Llama 3.3 from Meta through Groq's API
    ...
    try {
      ...
      chatCompletion = await getClient().chat.completions.create({
